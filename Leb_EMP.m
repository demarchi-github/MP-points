...
maxN=30;
n_vett=2:2:maxN;    % vector degree

ntg1mx = 10;      % the maximum value of the parameter determining 
...
for index = 1:length(n_vett)
    ...
    [xmp, ymp, wmp] =  ext_morr_patt (deg);
    mp = [xmp',ymp'];
    V_mp = chebvand2d(deg,mp);
   
    MV=V_mp'\V_leb';
    leb_mp(index) = norm(MV,1);
...
mesh(xtg,ytg,funLeb)
s=num2str(maxN);
title(strcat('Lebesgue function for n=', s))
figure
